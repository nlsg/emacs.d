...
If called interactively, enable Matlab-Shell-Inactive-Gud minor
...
With prefix ARG, enable Global Matlab-Shell-Inactive-Gud minor mode if ARG is positive;
otherwise, disable it.  If called from Lisp, enable the mode if
ARG is omitted or nil.

Matlab-Shell-Inactive-Gud minor mode is enabled in all buffers where
`(lambda nil Should we turn on in this buffer? Only if in a MATLAB mode. (when (eq major-mode 'matlab-mode) (matlab-shell-inactive-gud-minor-mode 1)))' would do it.
See `matlab-shell-inactive-gud-minor-mode' for more information on Matlab-Shell-Inactive-Gud minor mode.
...
(if (fboundp 'register-definition-prefixes) (register-definition-prefixes "mlgud" '("mlgud-")))
